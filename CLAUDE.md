...
`#prPageWrap` is sized in real px by `sizeReader()` from the booklet's own page aspect ratio (a spread is 2× one page's width), not a fixed box - recalculated on resize while the reader's open. prev/next/arrow-keys/swipe (same swipe pattern as Photography's `#camViewer`) drive `flipTo(position, dir)`: dir 1 (forward) animates the *right* slot's leaf away (`transform-origin:left`, the shared spine) to reveal the next spread; dir -1 (backward) mirrors it on the *left* slot (`transform-origin:right`). Both destination slots are swapped in before the leaf starts rotating, so once it passes 90° and its own face disappears (`backface-visibility:hidden`), the correct spread is already sitting there underneath. A `flipping` guard makes rapid clicks a no-op mid-turn rather than queuing or overlapping animations.
