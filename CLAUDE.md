...
### Stories quick-jump popup (hero edge)

Hovering the hero's orange "Stories" shape (`#storiesShapeTrigger`, the `<g>` inside `a[href="#stories"]`) pops up `#storiesPopup` at the hero's left edge - a direct-jump list to the five Stories sub-pages (Archives/Sketchbook/Graphics/3D/Photography, hand-written here same as the `#stories section .index` rows are, not generated from them). It's shown/hidden from *inside* the shared `onEnter(g)`/`onLeave(g)` shape-hover handlers, gated on `g.id === 'storiesShapeTrigger'`, rather than its own separate hover listeners - `onEnter`/`onLeave` already run identically for mouse hover *and* the touch two-tap arm/disarm flow (see Mobile section), so hooking in there covers both input modes with no extra touch-specific code. `resetAllOverlays()` also hides it, and popup items call `resetAllOverlays()` on click so navigating away from a popup item (which bypasses the shape itself) doesn't strand the hover state.
